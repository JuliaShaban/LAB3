...
public class MainFrame extends JFrame {
	...
	public MainFrame(Double[] coefficients) {
		super("Табулирование многочлена на отрезка по схеме Горнера");
		...
		Action aboutProgrammAction = new AbstractAction("О программе") {
			public void actionPerformed(ActionEvent event) {
				JOptionPane.showMessageDialog(MainFrame.this, "Автор: Шабан Юлия, 5 гр.");
			}
		};
		...
		buttonCalc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				try {
					...
					"Ошибка в формате числа с плавающей точкой", "Ошибочный формат числа", JOptionPane.WARNING_MESSAGE);
				}
			}
		});
	...
	protected void saveToTextFile(File selectedFile) {
		try {
			...
			out.println("====================================================");
			for (int i = 0; i<data.getRowCount(); i++) 
				out.println("Значения в точке  x = " + data.getValueAt(i,0).toString() + " равно " + data.getValueAt(i,1).toString());
			out.close();
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("Ошибка преобразовввания строки'" +
